...
function isSupabaseConfigured() {
  ...
  return !!(cfg.url && cfg.key && window.supabase?.createClient);
}

function createSupabaseClient() {
  const cfg = getSupabaseConfig();
  if (!cfg.url || !cfg.key) throw new Error("Supabase URL or publishable key is missing");
...
function stripTaskForCloud(task) {
  return {
    ...task,
...
function useSupabaseSync(tasks, setTasks, lists, setLists, configVersion = 0) {
  ...
  const applyPayload = useCallback((payload, deviceId) => {
    ...
    setStatus({ state: "on", text: "Синхронизировано" });
    setTimeout(() => {
  ...
  const pushSnapshot = useCallback(async () => {
    ...
    const payload = {
      tasks: tasksRef.current.map(stripTaskForCloud),
  ...
  const loadSnapshot = useCallback(async (client, currentUser) => {
    setStatus({ state: "syncing", text: "Загрузка..." });
    const { data, error } = await client.from(SUPABASE_TABLE).select("payload,device_id").eq("user_id", currentUser.id).maybeSingle();
    if (error) throw error;
    if (data?.payload) applyPayload(data.payload, data.device_id);
    else await pushSnapshot();
    readyRef.current = true;
    setStatus({ state: "on", text: "Синхронизация включена" });
  }, [applyPayload, pushSnapshot]);

  useEffect(() => {
    if (!isSupabaseConfigured()) {
      setStatus({ state: "warn", text: "Укажите Supabase URL" });
      return;
    }
    let alive = true;
    ...
    client.auth.getSession().then(({ data }) => {
      ...
      setStatus(data.session?.user ? { state: "syncing", text: "Подключение..." } : { state: "off", text: "Войдите в аккаунт" });
    });
    const { data: sub } = client.auth.onAuthStateChange((_event, session) => {
      if (!alive) return;
      readyRef.current = false;
      setUser(session?.user || null);
      setStatus(session?.user ? { state: "syncing", text: "Подключение..." } : { state: "off", text: "Войдите в аккаунт" });
    });
    return () => {
      alive = false;
  ...
  useEffect(() => {
    ...
    let channel = null;
    loadSnapshot(client, user).catch((e) => {
      console.error(e);
      setStatus({ state: "warn", text: "Ошибка синхронизации" });
    });
    channel = client
      .channel("todos-" + user.id)
      .on("postgres_changes", { event: "*", schema: "public", table: SUPABASE_TABLE, filter: "user_id=eq." + user.id }, (event) => {
        applyPayload(event.new?.payload, event.new?.device_id);
      })
      .subscribe();
    return () => {
      if (channel) client.removeChannel(channel);
    };
  }, [applyPayload, loadSnapshot, user]);

  useEffect(() => {
    if (!readyRef.current || applyingRef.current || !user) return;
    setStatus({ state: "syncing", text: "Сохранение..." });
    const id = setTimeout(() => {
  ...
  }, [tasks, lists, pushSnapshot, user]);

  const signIn = useCallback(async (email, password) => {
    const client = clientRef.current || createSupabaseClient();
    clientRef.current = client;
  ...
  const signOut = useCallback(async () => {
    ...
    readyRef.current = false;
    setUser(null);
    setStatus({ state: "off", text: "Войдите в аккаунт" });
  }, []);

  return { ...status, user, configured: isSupabaseConfigured(), signIn, signUp, signOut };
}
    ...
    /*#__PURE__*/ <Modal open={open} onClose={onClose} title={T.actions.sync}>
      ...
      {sync.user ? (
        <div className="ui-c-26">
          ...
          <div className="ui-c-22">
            <button disabled={busy} onClick={() => run(sync.signOut)} className="ui-c-31">{T.actions.signOut}</button>
          </div>
        </div>
      ) : (
        <div className="ui-c-26">
    ...
    /*#__PURE__*/ <aside
      ...
      <div className="ui-c-149">
        ...
        <button
          onClick={onOpenSync}
          className="ui-c-151">
          <L name={sync.user ? "CloudCheck" : sync.state === "syncing" ? "RefreshCw" : "Cloud"} size={18} />
          <span className="ui-c-110">{T.actions.sync}</span>
          <span className={`w-2 h-2 rounded-full ${sync.user ? "bg-green-500" : sync.state === "warn" ? "bg-amber-500" : "bg-gray-400"}`} />
        </button>
      </div>
    </aside>
  );
  ...
  return (
    /*#__PURE__*/ <div className="ui-c-170" onTouchStart={onSwipeStart} onTouchEnd={onSwipeEnd}>
      <Sidebar
        ...
        sync={sync}
        onOpenSync={() => setAccountOpen(true)}
        dark={dark}
